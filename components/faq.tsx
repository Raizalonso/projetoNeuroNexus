...
export default function FAQ() {
  const faqs = [
    ...
    {
      question: "Como sei se preciso de psicoterapia ou avaliação neuropsicológica?",
      answer:
    ...
    <section id="faq" className="py-20 relative">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16 fade-in">
          <h2 className="text-4xl lg:text-5xl font-serif font-bold title-shadow-right mb-4">PERGUNTAS FREQUENTES</h2>
          <p className="text-lg text-verde-titulo max-w-3xl mx-auto">
            Tire suas dúvidas sobre o processo terapêutico e os serviços oferecidos
          </p>
        </div>
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="border-b border-white/20">
                ...
                <AccordionContent className="text-white leading-relaxed pt-2">{faq.answer}</AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        </div>
