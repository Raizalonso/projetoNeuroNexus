  ...
  return (
    <section id="formacao" className="py-20 relative">
      ...
      <div className="absolute inset-0 z-0">
        <Image src="/images/fundo-formacao.png" alt="Fundo laranja formação" fill className="object-cover" />
        <div className="absolute inset-0 bg-white/5"></div>
      </div>

      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16 fade-in">
          <h2 className="text-4xl lg:text-5xl font-serif font-bold title-shadow-right mb-4">FORMAÇÃO E CURSOS</h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {credentials.map((credential, index) => {
            ...
            return (
              <div key={index} className="flex items-start slide-up" style={{ animationDelay: `${index * 0.1}s` }}>
                <div className="bg-[#3e5c4e] rounded-lg p-4 mr-4">
                  <IconComponent className="w-8 h-8 text-white" />
                </div>
                <div>
                  <h3 className="text-white font-serif font-bold text-lg mb-1">{credential.title}</h3>
